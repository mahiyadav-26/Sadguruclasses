...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (kind === "archive") {
      ...
      let upstreamArchive = await fetchRemoteFile(target, req.headers.get("range"));
      ...
      if ([403, 404, 410].includes(upstreamArchive.status)) {
        ...
        if (retryTarget) {
          upstreamArchive = await fetchRemoteFile(retryTarget, req.headers.get("range"));
        }
      }
      recordMetric({
...
async function resolveArchivePdfUrl(itemId: string): Promise<string | null> {
  ...
  // File choice matters for readability, not just speed. Archive derives an
  // "Additional Text PDF" (`*_text.pdf`) from scanned books: it carries the
  // invisible OCR text layer but NO page images, so it renders as blank white
  // pages in pdf.js — that was the real "Archive PDF open nahi hota" bug.
  // Prefer the original scan/container PDF; fall back to the text derivative
  // only when the item has nothing else.
  const isTextDerivative = (f: { name?: string; format?: string }) =>
    /_text\.pdf$/i.test(f.name || "") || /additional text pdf/i.test(f.format || "");
  const primary = files.filter((f) => !isTextDerivative(f));
  const pool = primary.length > 0 ? primary : files;
  // Within the chosen pool the smallest file is still the fastest to stream.
  pool.sort((a, b) => Number(a.size || 0) - Number(b.size || 0));
...
async function validatePdfResponse(upstream: Response, requestedRange: string | null): Promise<Response> {
  ...
  const shouldSniff = !requestedRange || /^bytes=0-/i.test(requestedRange.trim());
