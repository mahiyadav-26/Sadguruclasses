...
const AUTO_ALLOW_PATTERNS: RegExp[] = [
  ...
  /^http:\/\/localhost(:\d+)?$/i,
  /^http:\/\/127\.0\.0\.1(:\d+)?$/i,
  // Production web host (Vercel) + preview deployments. Without this the
  // helper falls back to ALLOWED[0] as soon as ALLOWED_ORIGINS is set, which
  // would break every payment call from the live website.
