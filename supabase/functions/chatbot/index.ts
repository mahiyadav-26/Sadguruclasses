...
const CHATBOT_AI_MODEL = 'google/gemini-3.5-flash';
...
function resolveChatbotModel(rawModel?: string | null): string {
  ...
  if (/preview|gemini-3-flash|gemini-2\.5-flash/i.test(model)) return CHATBOT_AI_MODEL;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const basePrompt = settings?.system_prompt ||
      `You are **Safar AI Agent**, the official AI learning companion for Sadguru Coaching Classes coaching platform.`;
...
2. Only introduce yourself when the user LITERALLY asks who you are (examples: "who are you", "aap kaun ho", "tum kaun ho", "kaun ho tum", "what is your name", "tumhara naam kya hai", "introduce yourself"). In that ONE case reply: "Main **Safar AI Agent** hoon – Sadguru Coaching Classes ka aapka 24×7 intelligent learning companion! 🤖🎓"
3. FOR EVERY OTHER MESSAGE: answer the question directly. DO NOT open with "Namaste", "Hello", "Hi", "Hey", "Main Safar AI Agent hoon", "Main aapka…", or any greeting / self-introduction. Skip pleasantries and go straight to the answer.
4. If abusive language: "Kripaya batchit ko sammanjanak rakhein. Main aapki poori madad karne ke liye yahan hoon. 🙏"
...
` + (queryInstructions[queryType] || '') + ragSection + webSection + studentContext + faqContext + courseContext;
    ...
    if (!isIdentityQuestion) {
      ...
      const introLine = /^\s*(main\s+\*?\*?safar\s+ai\s+agent\*?\*?\s+hoon[^\n]*|i\s+am\s+\*?\*?safar\s+ai\s+agent\*?\*?[^\n]*)\n+/i;
