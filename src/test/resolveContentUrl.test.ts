...
describe("extractContentPath", () => {
  ...
  it("parses legacy public HTTP URLs for THIS project", () => {
    expect(
      extractContentPath(
        "https://xvlvrbpqxqqqaeihofod.supabase.co/storage/v1/object/public/content/hero-banners/1.png"
      )
...
describe("resolveContentUrl", () => {
  ...
  it("signs public-folder legacy URLs too (content bucket is now private)", async () => {
    ...
    const url =
      "https://xvlvrbpqxqqqaeihofod.supabase.co/storage/v1/object/public/content/hero-banners/a.png";
  ...
  it("classifies a missing object quietly, not as sign_failed", async () => {
    ...
    const r = await resolveContentUrl("storage://content/thumbnails/gone-once.png");
    expect(r).toBeNull();
    expect(warn).not.toHaveBeenCalled();
    expect(info).toHaveBeenCalledWith(
      "[resolveContentUrl] missing object (placeholder used)",
      { path: "thumbnails/gone-once.png" }
    );
    warn.mockRestore();
    info.mockRestore();
  });
