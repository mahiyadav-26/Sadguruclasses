...
const ASK_TEACHERS = ["Raj VIP Sir", "English Sarthi", "Sahayak"];
...
export function useLessonChat(
  ...
  const invokeAI = useCallback(
    async (message: string, history: { role: string; content: string }[]) => {
      ...
      // Auto-retry once on transient gateway auth failure (fresh LOVABLE_API_KEY
      // may still be propagating to the edge function env).
      const apiCode = (data as { code?: string } | null)?.code;
      const status = (fnErr as { context?: { status?: number } })?.context?.status;
      if (apiCode === "gateway_unauthorized" || status === 503) {
      ...
      if (fnErr) {
        ...
        if (s === 429) throw new Error("Bahut requests aa gayi — thodi der baad try karo.");
        if (s === 402) throw new Error("AI credits khatam ho gaye. Admin ko batayein.");
        if (s === 503) throw new Error("AI service abhi reconnect ho raha hai. 10 second baad Retry dabao. 🙏");
        throw new Error(fnErr.message || "AI error");
      }
      const apiErr = (data as { error?: string; reply?: string; code?: string } | null)?.error;
      if (apiErr) {
        if ((data as { code?: string } | null)?.code === "gateway_unauthorized") {
          throw new Error("AI service abhi reconnect ho raha hai. 10 second baad Retry dabao. 🙏");
        }
