  ...
  const invokeAI = useCallback(
    async (message: string, history: { role: string; content: string }[]) => {
      ...
      if (fnErr) {
        ...
        if (s === 504) throw new Error("AI ne jawab dene me zyada time liya. Dobara Retry dabayein. 🙏");
        if (s === 503) throw new Error("AI service se connection nahi ho paaya. Kripaya Retry dabayein. 🙏");
        ...
        try {
          ...
          const j = (await ctx?.json?.()) as { error?: string } | undefined;
          if (typeof j?.error === "string") serverMsg = j.error;
        } catch { /* body wasn't JSON */ }
        throw new Error(serverMsg || "AI abhi jawab nahi de paaya. Retry dabayein. 🙏");
      }
      ...
      if (apiErr) {
        if ((data as { code?: string } | null)?.code === "gateway_unauthorized") {
          throw new Error("AI service configuration check zaroori hai. Admin ko batayein. 🙏");
        }
