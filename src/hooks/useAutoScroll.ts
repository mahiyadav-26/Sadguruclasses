      ...
      const step = (ts: number) => {
        ...
        if (max > 2) {
          ...
          if (el.dataset.archiveVirtualized === "true") {
            const pending = Array.from(el.querySelectorAll<HTMLElement>('[data-page-rendered="false"]'))
              .find((page) =>
                page.offsetTop >= el.scrollTop - 240 &&
                page.offsetTop <= el.scrollTop + el.clientHeight + 240
              );
            if (pending) {
          ...
          if (accRef.current >= 0.05) {
            ...
            el.scrollTop = Math.min(max, el.scrollTop + dy);
          }
