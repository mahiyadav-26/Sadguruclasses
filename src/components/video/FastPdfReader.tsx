...
import { isSheetsSource, isArchiveSource } from "../../lib/pdfSourceKind";
...
const FastPdfReader = forwardRef<FastPdfReaderHandle, Props>(
  ({ url, title, onSurfaceTap, onFirstByte, initialPage, onPageChange, onReady }, ref) => {
    ...
    // ── Total-size probe ────────────────────────────────────────────────
    // A single `Range: bytes=0-0` request returns `Content-Range: bytes 0-0/N`
    ...
    const onLoadProgress = useCallback(({ loaded, total }: { loaded: number; total: number }) => {
      ...
      pendingPct.current = effectiveTotal > 0
    ...
    if (resolving || (fallbackLoading && !file)) {
      return (
        <div className="absolute inset-0 bg-background">
          <ReaderProgress visible title={title} variant="pdf" />
        </div>
      );
    }
