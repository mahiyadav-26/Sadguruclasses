...
const ReaderProgress = memo(({ visible, title, variant = "pdf" }: Props) => {
  ...
  useEffect(() => {
    ...
    const onProgress = (e: Event) => {
      const detail = (e as CustomEvent<{ percent?: number; phase?: "downloading" | "rendering" | "ready" }>).detail;
    ...
    const ceiling = variant === "pdf" ? 40 : 90;
    const tau = variant === "pdf" ? 6 : 3;
  ...
  const phaseLabel = phase === "rendering" ? "Preparing pages" : baseLabel;
