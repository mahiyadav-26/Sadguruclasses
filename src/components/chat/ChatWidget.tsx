    ...
    try {
      ...
      if (error) {
        ...
        if (error instanceof FunctionsHttpError) {
          ...
          try {
            const j = (await error.context?.json?.()) as { error?: string } | undefined;
            serverMsg = j?.error;
      ...
      } else if (status === 401 || /unauthor|session/i.test(raw)) {
        friendly = "🔒 Session expire हो गया — page refresh करें।";
      } else if (status === 403 || /api_key|gateway/i.test(raw)) {
        friendly = "🔑 AI service अभी available नहीं है (server key issue). नीचे Retry दबाएँ — फिर भी न चले तो admin को बताएँ।";
      } else if (status && status >= 500) {
        friendly = "🛠️ Server पर दिक़्क़त है — थोड़ी देर बाद try करें।";
      }
